...
// ─── Tier 1: Deterministic AST Extraction ────────────────────────────────────
...
export interface EndpointRecord {
  ...
  // ─── Tier 2 Fields (added by data flow analysis) ─────────────────────
  ...
  // ─── Tier 3 Fields (added by LLM enrichment) ─────────────────────────
...
// ─── Tier 2: Data Flow Analysis Types ────────────────────────────────────────
...
// ─── JSON Schema (subset for pipeline use) ───────────────────────────────────
...
// ─── Pipeline Messages (IPC between extension host ↔ child process) ──────────

export type PipelineCommand =
  | { type: 'start'; workspaceRoot: string; options: PipelineOptions }
  | { type: 'cancel' }
  | { type: 'status' };

export type PipelineEvent =
  | { type: 'progress'; phase: PipelinePhase; message: string; percent: number }
  | { type: 'endpoints'; data: EndpointRecord[] }
  | { type: 'complete'; data: PipelineResult }
  | { type: 'error'; message: string; details?: string }
  | { type: 'log'; level: LogLevel; message: string };

export type PipelinePhase =
  | 'discovery'      // Finding files
  | 'detection'      // Detecting frameworks
  | 'tier1'          // AST scanning
  | 'tier2'          // Data flow analysis
  | 'tier3'          // LLM enrichment
